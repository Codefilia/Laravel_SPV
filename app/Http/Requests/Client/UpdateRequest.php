...
class UpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'ruc_number' => 'nullable|string|max:11|min:11|unique:clients,ruc_number,
            '.$this->route('client')->id.'',

            'address' => 'nullable|string|max:255|unique:clients',

            'phone_number' => 'required|string|max:9|min:9|unique:clients,phone_number,
            '.$this->route('client')->id.'',

            'email'=> 'nullable|email|string|max:200|email:rfc,dns|unique:clients,email,
            '.$this->route('client')->id.''
        ];
    }
